...
class HStackNode: SKSpriteNode {
    ...
    private var totalWidth: CGFloat = 0 // 容器的總寬度

    // 初始化，設置容器寬度
    init(containerWidth: CGFloat) {
        super.init(texture: nil, color: .clear, size: CGSize(width: containerWidth, height: 0))
    ...
    private func layoutElements() {
        ...
        for element in elements {
            totalElementWidth += element.size.width
        }
        ...
        for element in elements {
            // 計算每個元件的位置
            element.position = CGPoint(x: currentX + element.size.width / 2, y: 0)
